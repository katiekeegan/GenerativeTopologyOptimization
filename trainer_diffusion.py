...
Usage (example):
  python trainer_diffusion.py --ckpt checkpoints_mod/mod_last.pth --epochs 100 --batch-size 16
"""
import os
import argparse
import math
...
def train(args):
    ...
    if args.cond:
        ...
        dataset = VoxelSDFDataset(voxel_grids, problem_information_list = [F_list, Ω_design_list], num_query_points=args.num_query_points, fixed_surface_points_size=args.fixed_surface_points_size, noise_std=0.0, device=device, dataset="SELTO")
    else:
...
def parse_args():
    parser = argparse.ArgumentParser(description='Train a VE score model on VAE latents')
